const Home = () => {
  return (
    <>
      <div className="" style={{ marginTop: 100 }}>
        <p
          style={{
            textAlign: "center",
            fontSize: 25,
            marginLeft: 20,
            marginRight: 20,
          }}
        >
          🌙 Suivez le mouvement lunaire en temps réel ! 🌙
          <br />
          <br />
          Découvrez les phases de la Lune, sa position actuelle dans le ciel, et
          bien plus encore. Cette fonctionnalité vous permet de suivre
          l'évolution de la Lune au fil des jours. Fonctionnalités principales :
          Visualisation des phases lunaires (Nouvelle Lune, Pleine Lune, etc.).
          Suivi en direct de la trajectoire lunaire. Informations sur la
          distance et la luminosité de la Lune. Préparez-vous à explorer les
          mystères de notre satellite naturel ! 🌌
        </p>
      </div>
    </>
  );
};
export default Home;
